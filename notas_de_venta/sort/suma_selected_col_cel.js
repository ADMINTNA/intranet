...
// y selección múltiple no contigua (⌘/Ctrl + click).
...
(function(global){
  ...
  function initSumaSelectedColCel(selector){
    ...
    table.addEventListener("mousedown",e=>{
      ...
      // No interferir con selección múltiple (⌘/Ctrl)
    ...
    // === SELECCIÓN NO CONTIGUA (⌘/Ctrl + click) ===
    table.addEventListener("click",e=>{
      const cell=e.target.closest("td");
      if(!cell)return;

      // Si está en modo múltiple, agregar o quitar sin borrar otras
  ...
  function calculateSelectedCells(table){
    ...
    else showToast(`Mixto: ${res.numericCount} números, ${res.textCount} textos · Suma ${res.sum}`);
  }
