...
<?PHP 
    ...
    if(!empty($numero)) {
        ...
        if(!empty($categoria))  $cuales .= " && cc.tipo_caso_c   like '%".$categoria."%'";
        ...
        if(!empty($estado)) {
            ...
            if($estado == "abiertos") $cuales .= " && c.state != '%closed%'"; 
        }                
        ...
        if (isset($_POST["texto_avance"]) && trim($_POST["texto_avance"]) !== "") {
            $texto_avance = trim($_POST["texto_avance"]);
            $texto_avance = str_replace("'", "", $texto_avance);  // Elimina comillas simples
            $texto_avance = htmlspecialchars($texto_avance);      // Codifica HTML
            $cuales .= " && (
    ...
    $sql = "SELECT c.id			       as id,
               ...
               WHERE c.deleted = 0 
               && a.deleted = 0 
               && cc.categoria_c != 'Soporte_contrato_mensual'";
