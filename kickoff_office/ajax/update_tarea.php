...
// =========================================================
// UPDATE tasks_cstm (OPCIONAL - solo si existen las columnas)
// =========================================================
$sql2 = "
...
if ($stmt2) {
    ...
    // Intentar ejecutar, pero no fallar si las columnas no existen
    if (!$stmt2->execute()) {
        // Si falla por columnas inexistentes, continuar sin error
        $error_msg = $stmt2->error;
        if (strpos($error_msg, "Unknown column") === false) {
            // Solo fallar si NO es error de columna desconocida
            echo json_encode(["success"=>false,"error"=>"ERROR SQL2: ".$error_msg]);
            exit;
        }
        // Si es error de columna desconocida, ignorar y continuar
    }
}
