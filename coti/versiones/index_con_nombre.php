...
// Filtros a aplicar
$query_conditions = "WHERE aq.deleted = 0 AND aq.date_entered BETWEEN :start_date AND :end_date AND aqc.etapa_cotizacion_c NOT IN ('remplazada_cot', 'guia_oc_cli', 'orden_compra')";
...
// Consulta principal para obtener las cotizaciones con el campo nombre incluido
$sql = '
    SELECT 
        aq.number AS numero,
        aq.name AS nombre, 
        ...
        CASE 
            ...
            WHEN aqc.etapa_cotizacion_c = "cerrado_aceptado" THEN "Cerrado Única"
            ...
            WHEN aqc.etapa_cotizacion_c = "cerrado_aceptado_cli" THEN "Cerrado Bienal"
            WHEN aqc.etapa_cotizacion_c = "Borrador_cot" THEN "Borrador"
            WHEN aqc.etapa_cotizacion_c = "negociacion_cot" THEN "En Negociación"
        ...
        END AS Estado,
        CASE 
            WHEN cu.symbol = "USD" THEN "USD->UF"
            WHEN cu.symbol = "$" THEN "$->UF"  
            ELSE "UF"
        END AS Moneda,
        CASE 
    ...
    $query_conditions . ' ORDER BY aq.date_entered ASC';

// Preparar y ejecutar la consulta
$stmt = $pdo->prepare($sql);
$stmt->bindParam(':start_date', $start_date);
$stmt->bindParam(':end_date', $end_date);
$stmt->bindParam(':dolar', $dolar);
...
// Consultar los usuarios para el select de usuario según los registros encontrados
...
// Consultar los estados únicos para el select de estado según los registros encontrados
...
$resumen_sql = '
    ...
    LEFT JOIN tnasolut_sweet.aos_quotes_cstm AS aqc ON aq.id = aqc.id_c
    LEFT JOIN tnasolut_sweet.currencies AS cu ON aq.currency_id = cu.id ' .
    $query_conditions . ' GROUP BY Estado';

// Preparar y ejecutar la consulta con los parámetros dolar y uf
$resumen_stmt = $pdo->prepare($resumen_sql);
$resumen_stmt->bindParam(':start_date', $start_date);
$resumen_stmt->bindParam(':end_date', $end_date);
$resumen_stmt->bindParam(':dolar', $dolar); // Parámetro dolar
$resumen_stmt->bindParam(':uf', $uf);       // Parámetro uf
...
// Cálculo de ARPU por estado usando un bucle general para simplificar la lógica
$estado_montos = [
...
<head>
    <meta charset="UTF-8">
    <title>Análisis de Cotizaciones</title>
    <style>
...
<h1>Análisis de Cotizaciones</h1>
...
<?php if (!empty($quotes)): ?>
    <table align="center">
        <tr>
            <th>#</th>
            <th>Nº</th>
            <th>Nombre</th> <!-- Nueva columna para el campo nombre -->
        ...
        foreach ($quotes as $quote): ?>
            <tr>
                ...
                <td><?= htmlspecialchars($quote['nombre']); ?></td> <!-- Mostrar el campo nombre aquí -->
...
<?php if (!empty($resumen)): ?>
    ...
    <table class="resumen-table">
        <tr>
            ...
            <th class="resumen-title">% Distribución Cantidad</th>
            <th class="resumen-title">Suma del Monto</th>
            <th class="resumen-title">% Distribución Monto</th>
        </tr>
        <?php 
...
<!-- Visualización de ARPU por estado en una tabla horizontal -->
...
// Extraer datos del resumen para los gráficos
foreach ($resumen as $row) {
...
<script>
    ...
    new Chart(ctxCotizaciones, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Distribución de Cotizaciones',
