<?php
    function generarSelectVendedores() {
        ...
        if ($result && $result->num_rows > 0) {
            echo '<select multiple name="ejecutivo[]" id="ejecutivo" size="5">';
            while ($row = $result->fetch_assoc()) {
                ...
                if (!empty($ejecutivo) && strlen($ejecutivo) > 1 && !empty($id)) {
                    echo '<option value="' . htmlspecialchars($ejecutivo, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($ejecutivo, ENT_QUOTES, 'UTF-8') . '</option>';
                }
            }
            echo '</select>';
        } else {
