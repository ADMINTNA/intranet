...
<script  type="text/javascript">    
  ...
  function openWindowWithPostxx(url,name)
{
    alert("Entre");
    alert("url"+url);
    alert("name"+name);
...
function openWindowWithPost(pagina,titulo)
{
    alert("Entre");
    alert("pagina: "+pagina);
    alert("titulo: "+titulo);
    ...
    html += titulo;
    html += "+</title><body><form target='_self' id='formid' method='post' action='load_page.php'>";

    html += "<input type='hidden' name='pagina' value='" + pagina + "'/>";
    html += "<input type='hidden' name='titulo' value='" + titulo + "'/>";
...
<div class="scrollx">
	...
	<tr>
	    <td colspan="5" align="left" style="font-size: 14px; text-align: left;"><b><?php echo "Usuario: " . $_SESSION["name"] ?></b> [<a href="../login/logout.php">Salir</a>]</td>
	    <td colspan="5" align="left" style="font-size: 14px;"><a href="menu_no_visibles.php" alt="Aplicaciones No Visibles"> 
	...
	while($fila = mysqli_fetch_assoc($result)){ 
		...
		echo '<td><table class="xbox_app"><div>
				<tr>
					...
					<td>
					    <div align="center"><a href="javascript:openWindowWithPost(\''.$fila['url'].'\',\''.$fila['nombre'].'\')"; >'.$fila['nombre'].'</a></div>
					</td>
				</tr>
				<tr>
					<td colspan = "2"><div align="center" class="pseudolink" 
                    onclick="openWindowWithPost('.$fila['url'].')">                    
                    <img class="Redonda"  src="data:image/jpeg;base64,'.base64_encode( $fila['icono'] ).'" /></div></td>
				</tr>	
		</table></div></td>';
		if($ptr == 10 ) {
			$ptr = 0;
