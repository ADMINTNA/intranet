...
if($_POST['check']) {
	$update = 1;
	$query_update = 'UPDATE `app_x_cliente` SET `checked` = 1 WHERE `app_x_cliente`.`id_clie` = '.$_POST['id_clie'].' and  `app_x_cliente`.`id_app` = '.$_POST['check'];
}
...
if($_SESSION['TNA']) { 
	...
	/*$query = "select * from  app where id_app not in (";
	$query .= "select id_app from app_x_cliente  where id_clie = " . $id . " and checked = 1 ";
	$query .= ")";
	$query .= "order by app.nombre ASC";
	*/
		...
		$query .= "where cli.id = " . $id . " and cli_app.checked = 0 order by app.nombre ASC ";
}
if($update)	{
	$update=0;
...
<div class="scrollx">	
	...
	<tr>
	    <td colspan="5" align="left" style="font-size: 14px; text-align: left;"><b><?php echo "Usuario: " . $_SESSION["name"] ?></b></td>
	    <td colspan="5" align="left" style="font-size: 14px;"><a href="menumao.php">	
	        <img src="images/eye-visible.png"
